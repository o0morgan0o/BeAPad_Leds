...
struct SendMidiKeys_Fixture : public testing::Test {
    std::unique_ptr<SpecialEffect_Factory> specialEffectsFactory;
    std::unique_ptr<Debug_Helper> debugHelper;
    void SetUp() override {
        specialEffectsFactory = std::make_unique<SpecialEffect_Factory>();
        debugHelper = std::make_unique<Debug_Helper>();
    }
    ...
    public:
        Mock_Manager( LedBoard_Store_Interface *store, LightStrategy_Factory *factory, SpecialEffect_Factory *specialEffectFactory, Debug_Helper *debugHelper)
...
//    auto manager = std::make_unique<Mock_Manager>(store.get(), strategyFactory.get(), specialEffectsFactory.get(), debugHelper.get());
//    //
...
//    auto midiHandler = std::make_unique<Mock_MidiHandler>(midiReceiver.get(), midiSender.get(), debugHelper.get());
//    midiSender->connectBoardToSendMidiKey(0, 3, 60, 70);
//    midiSender->connectBoardToSendMidiKey(1, 4, 61, 71);
//    //
//    auto fakeSensor = std::make_unique<Fake_Sensor>();
//    auto dispatcher = std::make_unique<CapacitiveTouch_Dispatcher>(manager.get(), midiHandler.get(), debugHelper.get(), fakeSensor.get());
//    //
...
//    auto midiReceiver = std::make_unique<FakeMidiKeyReceiver>(manager.get(), debugHelper.get());
//    auto midiSender = std::make_unique<MidiKeySender>(manager.get(), debugHelper.get());
//    auto midiHandler = std::make_unique<Fake_MidiHandler>(midiReceiver.get(), midiSender.get(), debugHelper.get());
//    midiSender->connectBoardToSendMidiKey(0, 3, 60, 70);
//    midiSender->connectBoardToSendMidiKey(1, 4, 61, 71);
//    //
//    auto fakeSensor = std::make_unique<Fake_Sensor>();
